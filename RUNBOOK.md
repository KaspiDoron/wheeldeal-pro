...
| Webhook silent (403 storm) | Health panel `webhookSilent` alarm: outbound recent + session open + no inbound 30min | Almost always a stale token after a SESSION_SECRET or WEBHOOK_TOKEN_SALT change: the ping cycle re-arms via `reassertWebhook`; verify with Admin -> WA doctor. The re-arm is throttled to ~1h per instance, but a token change DEFEATS that throttle for EVERY open instance, not just the first one the sweep reaches: the fleet-wide `WH_REARM_TOKEN_FP` row records which token was last registered AND the moment it came into force, and any instance whose own `wa_sessions.webhook_rearmed_at` predates that moment re-registers. The sweep runs one minute in five over up to 50 open instances, so a rotated fleet is repaired within minutes, not up to an hour later. |
...
| Owner locked out | Wrong password + lockout, or blocked by accident | `node scripts/admin-recover-owner.mjs --password '<8+ chars>'` - sets that scrypt password on `app_users` (no built-in default; `OWNER_BOOTSTRAP_PASSWORD` in the env also works), unblocks, clears lockouts. Needs service-role env locally. |
