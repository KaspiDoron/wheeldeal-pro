...
export async function getTrafficConfig(): Promise<TrafficConfig> {
  const [modeRaw, partnersRaw, cmpRaw] = await Promise.all([
  ...
  return { mode, cmp, partners, errors };
}
...
export function toPublicTraffic(config: TrafficConfig, market = "xx"): PublicTrafficConfig {
