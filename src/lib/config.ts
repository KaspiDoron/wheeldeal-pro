...
const KEYS: {
  ...
  { name: "EVOLUTION_HOSTS", label: "Evolution host pool (url|key[|dial prefixes][|cap] per line)", scope: "messaging", editable: true },
  { name: "EVOLUTION_MAX_PER_HOST", label: "Max WhatsApp users per host (fleet DEFAULT - a host's own 4th field wins)", scope: "messaging", editable: true, secret: false },
  ...
  { name: "WABA_ACCOUNT_ID", label: "WhatsApp Business ACCOUNT id (Meta only) - lets the key test confirm the template name and language are approved BEFORE the first real send", scope: "messaging", editable: true, secret: false },
  { name: "WABA_WEBHOOK_SECRET", label: "Business API webhook SIGNING secret - for Meta this is the APP SECRET (verifies X-Hub-Signature-256)", scope: "messaging", editable: true },
  { name: "WABA_VERIFY_TOKEN", label: "Business API webhook VERIFY token - any string you also type into Meta's callback form. Blank falls back to the signing secret; never reuse the app secret, this value travels in a URL and lands in access logs", scope: "messaging", editable: true },
  { name: "WABA_TEMPLATE_FIRST_CONTACT", label: "Approved first-contact template name", scope: "messaging", editable: true, secret: false },
  { name: "WABA_TEMPLATE_REENGAGE", label: "Approved re-engagement template name - RESERVED, nothing sends it yet (no code path re-engages a quiet agency)", scope: "messaging", editable: true, secret: false },
  { name: "WABA_TEMPLATE_LANGUAGE", label: "Approved template LANGUAGE code, exactly as Meta lists it (en, en_US, th). Blank means en. A mismatch is error 132001 on every send", scope: "messaging", editable: true, secret: false },
